...
|----------|---------------------------------------------------|------------------|
| Windows  | `Parlys-Setup-<version>-x64.exe` (NSIS)         | ✅ Automatic (Desktop + Start Menu) |
| Linux    | `Parlys-<version>-x86_64.AppImage` (portable)   | Via AppImageLauncher (optional) or `chmod +x && ./Parlys.AppImage --appimage-extract` |
| Linux    | `parlys_<version>_amd64.deb` (Debian / Ubuntu)  | ✅ Automatic — a **Parlys.desktop** icon is dropped on every user's Desktop **and** registered in the application menu |
| Linux    | `parlys-<version>.x86_64.rpm` (Fedora / RHEL)   | ✅ Automatic — same mechanism as the .deb |
| Linux    | `parlys-<version>-linux-x64.tar.gz` (portable)  | Manual (unpack anywhere, run `./parlys`) |
| macOS    | `Parlys-<version>-mac-(x64\|arm64).dmg`         | Drag `Parlys.app` into **Applications** — the icon appears automatically in Launchpad and Spotlight |
| macOS    | `Parlys-<version>-mac-(x64\|arm64).zip`         | Same as DMG, but unpacks faster without mounting a volume |
...
npm ci
npm run dist:win
# → release\Parlys-Setup-1.0.0-x64.exe
```
...
npm ci
npm run dist:linux
# → release/Parlys-1.0.0-x86_64.AppImage
# → release/parlys_1.0.0_amd64.deb
# → release/parlys-1.0.0.x86_64.rpm
# → release/parlys-1.0.0-linux-x64.tar.gz
```

**Desktop shortcut behaviour:** the `.deb` and `.rpm` packages run `build/linux-after-install.sh` during installation, which:
- registers the app menu entry via `/usr/share/applications/parlys.desktop`
- drops a `Parlys.desktop` launcher on **every existing user's Desktop** (respecting XDG `user-dirs.dirs`, falling back to `~/Desktop` or `~/Bureau`)
...
npm ci
npm run dist:mac
# → release/Parlys-1.0.0-mac-x64.dmg
# → release/Parlys-1.0.0-mac-arm64.dmg
# → release/Parlys-1.0.0-mac-x64.zip
# → release/Parlys-1.0.0-mac-arm64.zip
```
...
- **macOS** — Gatekeeper will block the app. Right-click → *Open* once, or run `xattr -cr /Applications/Parlys.app`. For a permanent fix you need an Apple Developer ID and an app-specific password set via `CSC_LINK` / `APPLE_ID` / `APPLE_APP_SPECIFIC_PASSWORD`.

## Uninstall

- **Windows** — *Settings → Apps* → Parlys → Uninstall. User settings (in `%APPDATA%\parlys`) are preserved unless you tick *Delete user data*.
- **Linux (.deb)** — `sudo apt remove parlys` (also removes the Desktop shortcut).
- **Linux (.rpm)** — `sudo dnf remove parlys` or `sudo rpm -e parlys`.
- **Linux (AppImage)** — delete the file; AppImageLauncher removes its registration automatically.
- **macOS** — drag `Parlys.app` out of Applications. Settings live in `~/Library/Application Support/parlys`.
